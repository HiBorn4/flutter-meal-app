...
class MealDetailScreen extends StatelessWidget {
  ...
  final Function _isMealFavourite; // A function to check if a meal is favourite.
  ...
  Widget buildSectionTitle (BuildContext context, String text) {
    return SizedBox(
      child: Text(
      text,
      style: Theme.of(context).textTheme.titleMedium, // Styling for the text as per the current theme.
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    final mealId = ModalRoute.of(context).settings.arguments as String; // Getting the meal ID from the navigation arguments.
    final selectedMeal = DUMMY_MEALS.firstWhere((meal) => meal.id == mealId); // Getting the meal object using the meal ID.

    // Creating a Scaffold widget for the screen with required elements.
    return Scaffold(
      appBar: AppBar(title: Text(selectedMeal.title),), // App bar with the title of the selected meal.
      body: SingleChildScrollView( // Scrollable container for the body.
        child: Column(
          children: <Widget>[
            SizedBox(
              height: 300, // Height for the image container.
              width: double.infinity, // Width for the image container.
              child: Image.network( // Image widget for displaying the meal image.
                selectedMeal.imageUrl, // URL for the image.
                fit: BoxFit.cover, // Fitting the image within the container.
              ),
            ),
            buildSectionTitle(context, "Ingredients"), // Section title for ingredients.
            buildContainer(
              ListView.builder( // ListView for displaying the list of ingredients.
                itemBuilder: (ctx, index) => Card(
                  color: Theme.of(context).colorScheme.secondary, // Styling for the card background color.
                  child: Padding(padding: const EdgeInsets.symmetric(
                    vertical: 5,
                    horizontal: 10,
                  ),child: Text(selectedMeal.ingredients[index])), // Displaying the text for the ingredient.
                ),
                itemCount: selectedMeal.ingredients.length, // Total number of ingredients in the list.
              )
            ),
            buildSectionTitle(context, "Steps"), // Section title for steps.
            buildContainer(ListView.builder( // ListView for displaying the list of steps.
      ...
      floatingActionButton: FloatingActionButton( // This line of code creates a FloatingActionButton
        onPressed: () => favourite(mealId), // widget that allows the user to add the selected
        child: Icon(  // meal to their favorites list.
